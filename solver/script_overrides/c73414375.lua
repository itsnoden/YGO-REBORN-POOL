-- YGO Reborn reviewed override for Project Ignis official/c73414375.lua
-- Dimension Jar (73414375)
--
-- Upstream baseline reviewed: ProjectIgnis/CardScripts
--   14064037f42d0ae0bb9f8577d7eba2baad95e633
--
-- Current official English text (Yu-Gi-Oh! TCG Card Database, cid=5500):
-- "FLIP: Both players can remove from play up to 3 monsters from their opponent's Graveyard."
--
-- Upstream's s.rmfilter uses aux.SpElimFilter(c) without first requiring a
-- Monster. For non-Monster cards in the GY, SpElimFilter returns true, so the
-- upstream script can incorrectly allow Spells/Traps to be selected and banished.
--
-- Reborn correction: require c:IsMonster() before the existing removal and
-- Spirit Elimination compatibility checks. No other behavior is changed.
